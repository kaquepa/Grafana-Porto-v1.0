...
# 2. Health checks
echo "🏥 === HEALTH CHECKS ==="
docker inspect postgres_database --format='{{json .State.Health}}' | python3 -m json.tool 2>/dev/null || echo "PostgreSQL health check não disponível"
docker inspect grafana_dashboard --format='{{json .State.Health}}' | python3 -m json.tool 2>/dev/null || echo "Grafana health check não disponível"
echo ""

# 3. Conectividade de rede
echo "🌐 === TESTE DE CONECTIVIDADE ==="
echo "Testando conexão do Grafana para PostgreSQL..."
docker exec grafana_dashboard sh -c "nc -zv postgres_database 5432" 2>&1 || echo "❌ Falha na conectividade"
echo ""
...
docker exec grafana_dashboard sh -c 'echo "POSTGRES_PASSWORD: $POSTGRES_PASSWORD"'
echo ""
