...
def execute():
    """ABORDAGEM RADICAL - Foca no que FUNCIONA"""
    logger.info("🚀 INICIANDO ABORDAGEM RADICAL...")
    ...
    try:
        ...
        # 🔥 MÉTODO RADICAL
        logger.info("🔌 APLICANDO MÉTODO RADICAL NO DATASOURCE...")
        ds_uid = manager.create_datasource("postgres-porto-uid")
        
        # ✅ VERIFICAÇÃO PRÁTICA
        logger.info("🔍 VERIFICAÇÃO PRÁTICA...")
        
        # Teste DIRETO - se as queries funcionam
        test_queries = [
            "SELECT COUNT(*) as total FROM operations",
            "SELECT COUNT(*) as waiting FROM vessel_queue WHERE status = 'waiting'",
            "SELECT COUNT(*) as occupied FROM berths WHERE status = 'occupied'"
        ]
        
        working_queries = 0
        for query in test_queries:
            if manager._execute_direct_query_test(ds_uid, query):
                working_queries += 1
                logger.info(f"✅ Query funcional: {query[:50]}...")
        
        logger.info(f"📊 Queries funcionais: {working_queries}/{len(test_queries)}")
        
        # 📊 CRIA DASHBOARD MESMO COM AVISOS
        logger.info("📊 CRIANDO DASHBOARD (mesmo com avisos)...")
        ...
        dashboard_result = manager.create_dashboard(config, panels)
        
        # 🎉 RELATÓRIO HONESTO
        print(f"\n{'='*80}")
        print("🎉 CONFIGURAÇÃO RADICAL CONCLUÍDA!")
        print(f"✅ DataSource: {ds_uid}")
        print(f"✅ Queries funcionais: {working_queries}/{len(test_queries)}")
        print(f"✅ Dashboard criado: {config.title}")
        print(f"🔗 URL: {Config_grafana.URL}/d/{config.uid}")
        print("\n💡 SE OS DADOS NÃO APARECEM:")
        print("1. Acesse: Data Sources → PostgreSQL-Porto-Operacional")
        print("2. Clique em 'Save & Test' UMA vez")
        print("3. Os painéis começarão a mostrar dados automaticamente")
        print(f"{'='*80}\n")
        
        return 0
        
    except Exception as e:
        logger.error(f"💥 FALHA RADICAL: {e}")
        return 1
