...
class DashboardManager:
    ...
    def test_datasource_with_query(self, ds_uid: str) -> bool:
        """
        Testa o datasource com uma query SQL real (o verdadeiro Save & Test).
        """
        try:
            # Query simples que deve funcionar em qualquer PostgreSQL
            ...
            if "results" in result:
                ...
                if frames and len(frames) > 0:
                    logger.info(f"✅ Teste SQL passou: query executada com sucesso")
                    return True
            
            logger.error(f"❌ Teste SQL falhou: resposta inválida - {result}")
            return False
            
        except Exception as e:
            logger.error(f"❌ Erro ao testar datasource com query: {e}")
            if hasattr(e, 'response') and e.response:
                logger.error(f"Response: {e.response.text}")
            return False
    ...
    def get_or_create_datasource(self, uid: str, retries: int = 3, delay: int = 5) -> str:
        """
        Verifica se datasource existe. Se não, cria e testa COM QUERY REAL.
        """
        config = {
            "uid": uid,
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                try:
                    ...
                    logger.info(f"Datasource {uid} encontrado")
                    
                    # Atualiza configuração (força senha correta)
                    config["id"] = ds_id
                    self.api.put(f"/datasources/{ds_id}", json=config)
                    logger.info(f"Datasource {uid} atualizado")
                    
                    # Aguarda um pouco para aplicar mudanças
                    time.sleep(2)
                    
                    # TESTE REAL COM QUERY
                    if self.test_datasource_with_query(uid):
                        logger.info(f"✅ Datasource {uid} configurado e testado com sucesso")
                        return uid
                    else:
                        logger.error(f"❌ Datasource existe mas teste com query falhou")
                        logger.error("Verifique manualmente: credenciais, host, porta, database")
                        # Retorna mesmo assim para não bloquear - mas avisa
                        return uid
                    ...
                    # Aguarda e testa com query
                    time.sleep(3)
                    
                    if self.test_datasource_with_query(uid):
                        logger.info(f"✅ Datasource criado e testado com sucesso")
                        return uid
                    else:
                        logger.error(f"❌ Datasource criado mas teste falhou")
                        return uid
