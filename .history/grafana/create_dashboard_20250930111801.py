...
class DashboardManager:
    ...
    def create_datasource(self, uid: str, max_retries: int = 3) -> str:
        """
        Cria datasource via API com delete + recreate para garantir credenciais corretas.
        Esta é a ÚNICA forma confiável de aplicar secureJsonData (senha).
        """
        config = {
            "uid": uid,
        ...
        for attempt in range(1, max_retries + 1):
            try:
                # Tenta deletar datasource existente (pode ser provisionado ou antigo)
                try:
                    response = self.api.get(f"/datasources/uid/{uid}")
                    ds_id = response.json()["id"]
                    self.api.delete(f"/datasources/{ds_id}")
                    logger.info(f"Datasource existente {uid} deletado")
                    time.sleep(2)
                except requests.HTTPError as e:
                    if e.response.status_code == 404:
                        logger.info(f"Nenhum datasource anterior encontrado")
                    else:
                        raise
                
                # Cria datasource do zero
                logger.info(f"Criando datasource {uid} via API...")
                response = self.api.post("/datasources", json=config)
                logger.info(f"Datasource {uid} criado com sucesso")
                
                # Aguarda aplicação das credenciais
                time.sleep(3)
                ...
                if self._test_datasource_query(uid):
                    ...
                    logger.warning(f"Tentativa {attempt}: datasource criado mas teste falhou")
                    if attempt < max_retries:
                        time.sleep(3)
                    
            except requests.ConnectionError:
                logger.warning(f"Tentativa {attempt}: Grafana não acessível")
                if attempt < max_retries:
                    time.sleep(5)
                else:
        ...
        raise RuntimeError(f"Não foi possível criar datasource funcional após {max_retries} tentativas")
