...
class DashboardManager:
    ...
    def test_datasource_with_query(self, ds_uid: str) -> bool:
        ...
        try:
            ...
            return False
    def test_datasource_with_query(self, ds_uid: str) -> bool:
        """Testa datasource executando uma query SQL real"""
        try:
            payload = {
                "queries": [{
                    "refId": "A",
            ...
            logger.error(f"Teste SQL falhou: {result}")
            return False
    ...
    def get_or_create_datasource(self, uid: str, retries: int = 3, delay: int = 5) -> str:
        """
        Verifica se datasource existe. Se sim, deleta e recria para aplicar credenciais.
        Isso força o Grafana a recarregar as configurações (equivalente ao Save & Test).
        """
        config = {
            "uid": uid,
        ...
        for attempt in range(1, retries + 1):
            try:
                try:
                    ...
                    logger.info(f"Datasource {uid} existe, recriando para aplicar credenciais...")
                    
                    # DELETA o datasource existente
                    self.api.delete(f"/datasources/{ds_id}")
                    logger.info(f"Datasource {uid} deletado")
                    time.sleep(2)
                    
                except requests.HTTPError as e:
                    if e.response.status_code != 404:
                        raise
                    logger.info(f"Datasource {uid} não existe")
                
                # CRIA o datasource (sempre, seja novo ou recriado)
                ...
                # TESTA com query real
                if self.test_datasource_with_query(uid):
                    logger.info(f"Datasource {uid} testado e funcionando")
                    return uid
                else:
                ...
                if attempt < retries:
                    ...
                    raise RuntimeError("Grafana não acessível")
