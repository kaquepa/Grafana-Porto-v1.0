...
class DashboardManager:
    ...
    #---------------------------------------
    def force_datasource_reload(self, uid: str) -> bool:
        """
        Força o Grafana a recarregar o datasource (equivalente a Save & Test na UI).
        Isso é necessário porque atualizar secureJsonData via PUT não aplica imediatamente.
        """
        try:
            # 1. Busca o datasource
            response = self.api.get(f"/datasources/uid/{uid}")
            ds_data = response.json()
            ds_id = ds_data["id"]
            
            # 2. Faz um PUT mesmo que não mude nada - isso força reload
            # Remove campos que o Grafana não aceita no PUT
            ds_data.pop('readOnly', None)
            ds_data.pop('version', None)
            
            self.api.put(f"/datasources/{ds_id}", json=ds_data)
            logger.info(f"Datasource {uid} recarregado")
            
            # 3. Aguarda um pouco
            time.sleep(2)
            
            # 4. Testa com query real
            return self.test_datasource_with_query(uid)
            
        except Exception as e:
            logger.error(f"Erro ao recarregar datasource: {e}")
            return False


    def get_or_create_datasource(self, uid: str, retries: int = 3, delay: int = 5) -> str:
        """
        Verifica se datasource existe. Se sim, força reload das credenciais.
        """
        config = {
            "uid": uid,
        ...
        for attempt in range(1, retries + 1):
            try:
                try:
                    ...
                    logger.info(f"Datasource {uid} encontrado, atualizando credenciais...")
                    
                    # IMPORTANTE: Atualizar com secureJsonData
                    config["id"] = ds_id
                    self.api.put(f"/datasources/{ds_id}", json=config)
                    
                    # TRUQUE: Deleta e recria para forçar aplicação das credenciais
                    logger.info("Forçando reload das credenciais...")
                    
                    # Salva a config atual
                    old_config = config.copy()
                    
                    # Deleta
                    self.api.delete(f"/datasources/{ds_id}")
                    time.sleep(1)
                    
                    # Recria com as mesmas configs
                    response = self.api.post("/datasources", json=old_config)
                    logger.info(f"Datasource {uid} recriado com credenciais atualizadas")
                    
                    time.sleep(3)
                    
                    # Testa
                    if self.test_datasource_with_query(uid):
                        logger.info(f"✅ Datasource {uid} funcionando corretamente")
                        return uid
                    else:
                        logger.error("❌ Teste falhou após recriar datasource")
                        return uid
                    ...
                    if self.test_datasource_with_query(uid):
                        logger.info(f"✅ Datasource criado e testado")
                        return uid
                    else:
                        logger.error("❌ Teste falhou no datasource novo")
                        return uid
                        
            except requests.ConnectionError as e:
                logger.warning(f"Tentativa {attempt}/{retries}: Grafana não acessível")
                if attempt < retries:
                    time.sleep(delay)
                else:
