...
class DashboardManager:
    ...
    def test_datasource(self, uid: str, config: dict = None):
        """
        Salva e testa o datasource no Grafana (equivalente ao 'Save & Test' da UI).

        :param uid: UID do datasource
        :param config: Configuração completa do datasource (opcional, usado para atualizar)
        :return: True se o datasource estiver OK, False caso contrário
        """
        try:
            # 1️⃣ Obter datasource existente
            response = self.api.get(f"/datasources/uid/{uid}")
            response.raise_for_status()
            data = response.json()
            ds_id = data["id"]

            # 2️⃣ Atualizar configuração, se fornecida
            if config:
                self.api.put(f"/datasources/{ds_id}", json=config)
                logger.info(f"Datasource {uid} atualizado com PUT")

            # 3️⃣ Fazer Save & Test correto
            test_payload = config if config else data
            response_test = self.api.post("/datasources/test", json=test_payload)
            response_test.raise_for_status()
            logger.info(f"Datasource {uid} salvo e testado com sucesso")
            return True

        except requests.RequestException as e:
            logger.error(f"Erro ao salvar/testar datasource {uid}: {e}")
            if hasattr(e, 'response') and e.response:
                logger.error(f"Response: {e.response.text}")
            return False
    def save_and_test_datasource(self, config: dict) -> bool:
        """
        Equivalente ao botão 'Save & Test' da UI do Grafana.
        Cria/atualiza o datasource e testa a conexão.
        """
        uid = config.get("uid")
        
        try:
            # 1️⃣ Verifica se datasource já existe
            try:
                response = self.api.get(f"/datasources/uid/{uid}")
            ...
            # 2️⃣ SAVE: Cria ou atualiza
            if exists:
                config["id"] = ds_id
                response = self.api.put(f"/datasources/{ds_id}", json=config)
                logger.info(f"Datasource {uid} atualizado")
            else:
                response = self.api.post("/datasources", json=config)
                ds_id = response.json()["datasource"]["id"]
                logger.info(f"Datasource {uid} criado")
            
            # 3️⃣ TEST: Testa a conexão (equivalente ao botão Test)
            time.sleep(1)  # Aguarda um pouco para o datasource estar pronto
            
            test_response = self.api.get(f"/datasources/{ds_id}/health")
            result = test_response.json()
            
            if result.get("status") == "OK":
                logger.info(f"✅ Save & Test concluído: {result.get('message', 'Conexão OK')}")
                return True
            else:
                logger.error(f"❌ Teste falhou: {result}")
                return False
                
        except Exception as e:
            logger.error(f"❌ Erro no Save & Test: {e}")
            if hasattr(e, 'response') and e.response:
                logger.error(f"Response: {e.response.text}")
            return False
    
    def _get_or_create_datasource(self, uid: str, api_key: str, retries: int = 10, delay: int = 30) -> str:
        """
    ...
    def get_or_create_datasource(self, uid: str, retries: int = 3, delay: int = 5) -> str:
        """
        Verifica se datasource existe. Se não, cria e testa.
        Faz retry apenas se Grafana não estiver acessível.
        """
        config = {
            "uid": uid,
            "name": "grafana-postgresql-datasource",
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                try:
                    ...
                    logger.info(f"Datasource {uid} existe")
                    
                    # Atualiza para garantir que está como default
                    config["id"] = ds_id
                    self.api.put(f"/datasources/{ds_id}", json=config)
                    logger.info(f"Datasource {uid} atualizado como default")
                    
                    # Testa conexão
                    if self.test_datasource_health(ds_id):
                        return uid
                    else:
                        logger.warning("Datasource existe mas teste de conexão falhou")
                        return uid  # Retorna mesmo assim, pode ser problema temporário
                        
                except requests.HTTPError as e:
                    if e.response.status_code != 404:
                        raise  # Erro diferente de "não encontrado"
                    
                    # 2️⃣ Datasource não existe, cria
                    logger.info(f"Datasource {uid} não encontrado, criando...")
                    response = self.api.post("/datasources", json=config)
                    result = response.json()
                    ds_id = result.get("datasource", {}).get("id") or result.get("id")
                    
                    logger.info(f"Datasource {uid} criado com sucesso")
                    
                    # Aguarda um momento e testa
                    time.sleep(2)
                    if self.test_datasource_health(ds_id):
                        return uid
                    else:
                        logger.warning("Datasource criado mas teste falhou")
                        return uid
                        
            except requests.ConnectionError as e:
                # Grafana não está acessível - retry faz sentido
                logger.warning(f"Tentativa {attempt}/{retries}: Grafana não acessível - {e}")
                if attempt < retries:
                    logger.info(f"Aguardando {delay}s antes de tentar novamente...")
                    time.sleep(delay)
                else:
                    raise RuntimeError(f"Grafana não acessível após {retries} tentativas")
                    
            except Exception as e:
                # Outros erros - não faz sentido retry
                logger.error(f"Erro ao configurar datasource: {e}")
                raise
        
        raise RuntimeError(f"Não foi possível configurar datasource {uid}")


    def test_datasource_health(self, ds_id: int) -> bool:
        """
        Testa a saúde do datasource (equivalente ao botão Test).
        """
        try:
            response = self.api.get(f"/datasources/{ds_id}/health")
            result = response.json()
            ...
            if status == "OK":
                logger.info(f"✅ Teste de conexão OK: {message}")
                return True
            else:
                logger.error(f"❌ Teste de conexão falhou: {status} - {message}")
                return False
                
        except Exception as e:
            logger.error(f"❌ Erro ao testar datasource: {e}")
            return False
...
def execute():
    ...
    ds_uid = manager.get_or_create_datasource("postgres-porto-uid")
