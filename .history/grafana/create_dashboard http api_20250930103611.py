...
class DashboardManager:
    ...
    def create_datasource(self, uid: str, max_retries: int = 5) -> str:
        """Cria e ativa completamente o datasource"""
        datasource_config = {
            "uid": uid,
        ...
        for attempt in range(1, max_retries + 1):
            try:
                logger.info(f"🔄 Tentativa {attempt}/{max_retries} para datasource {uid}")
                
                # 1. Limpeza: Remove datasource existente
                try:
                    ...
                    if e.response.status_code == 404:
                        logger.info("📭 Nenhum datasource anterior encontrado")
                    else:
                        logger.warning(f"⚠️ Erro ao verificar datasource: {e}")
                
                # 2. Criação: Novo datasource
                logger.info("🆕 Criando novo datasource...")
                ...
                # 4. ✅ ATIVAÇÃO CRÍTICA: Simula "Save & Test"
                if self._activate_datasource(uid):
                    logger.info("🎯 Datasource ativado com sucesso!")
                    return uid
                else:
                    logger.warning(f"❌ Falha na ativação (tentativa {attempt})")
                    if attempt < max_retries:
                        wait_time = attempt * 3
                        logger.info(f"⏰ Nova tentativa em {wait_time}s...")
                        time.sleep(wait_time)
                    
            except requests.ConnectionError:
                logger.error(f"🌐 Erro de conexão (tentativa {attempt})")
                if attempt < max_retries:
                    time.sleep(10)
                else:
                    raise RuntimeError("Grafana inacessível após múltiplas tentativas")
        ...
        raise RuntimeError(f"❌ Não foi possível criar datasource após {max_retries} tentativas")
