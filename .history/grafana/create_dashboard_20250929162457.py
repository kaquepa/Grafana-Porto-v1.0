...
class DashboardManager:
    ...
    def get_or_create_datasource(self, uid: str, api_key: str) -> str:
        """
        ...
        response = self.api.get(f"/api/datasources/uid/{uid}", headers=headers)
        if response.status_code == 200:
            data = response.json()
            ds_id = data["id"]  # id numérico
            logger.info(f"Datasource {uid} existe, atualizando para default...")
            self.api.put(f"/api/datasources/{ds_id}", json=config, headers=headers)
        ...
        response = self.api.post("/api/datasources", json=config, headers=headers)
    ...
    response = self.api.get(f"/api/datasources/uid/{uid}", headers=headers)  # Removido /api duplicado
    if response.status_code == 200:
        data = response.json()
        ds_id = data["id"]  # id numérico
        logger.info(f"Datasource {uid} existe, atualizando para default...")
    ...
    response = self.api.post("/api/datasources", json=config, headers=headers)  # Removido /api duplicado
    if response.status_code in [200, 201]:
        logger.info(f"Datasource {uid} criado com sucesso e definido como default")
        self.test_datasource(uid)
        return uid
