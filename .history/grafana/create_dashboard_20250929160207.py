...
class DashboardManager:
    ...
    def test_datasource(self, uid: str) -> bool:
        """Testa conexão com datasource"""
        try:
            response = self.api.post("/ds/query", json={
                "queries": [{"refId": "A", "datasource": {"uid": uid}, "rawSql": "SELECT 1", "format": "table"}]
            })
            logger.info(f"Datasource {uid} testado com sucesso")
            return response.status_code == 200
        except Exception as e:
            logger.error(f"Falha ao testar datasource {uid}: {e}")
            return False
    ...
    def get_or_create_datasource(self, uid: str) -> str:
        """
        ...
        try:
            response = self.api.get(f"/api/datasources/uid/{uid}")
            if response.status_code == 200:
                logger.info(f"Datasource {uid} já existe, atualizando para default...")
                # Atualiza configuração existente
                self.api.put(f"/api/datasources/{uid}", json=config)
                time.sleep(1)
        ...
        try:
            response = self.api.post("/api/datasources", json=config)
            ...
            logger.error(f"Erro inesperado ao criar datasource: {e}")
            raise
