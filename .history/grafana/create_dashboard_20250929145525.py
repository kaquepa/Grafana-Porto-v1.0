...
class DashboardManager:
    ...
    def get_or_create_datasource(self, uid: str) -> str:
        """
        Verifica se datasource existe (provisionado ou via API).
        Só cria se não encontrar.
        """
        try:
            # Tenta buscar datasource existente
            response = self.api.get(f"/datasources/uid/{uid}")
            if response.status_code == 200:
                logger.info(f"Datasource {uid} encontrado (provisionado ou existente)")
                self.test_datasource(uid)
                return uid
        except requests.HTTPError as e:
            if e.response.status_code != 404:
                raise
            logger.info(f"Datasource {uid} não encontrado, será criado via API")
        ...
        try:
            self.api.post("/datasources", json=config)
    ...
    def create_panels(self, ds_uid: str) -> List[Dict[str, Any]]:
        ...
        panels = []
        ds_uid = "grafana-postgresql-datasource"
