...
def _execute():
    ...
    logger.info("🧪 TESTE RÁPIDO DO DATASOURCE...")
    ...
    try:
        ...
        # 1. Tenta criar datasource de forma simples
        logger.info("1. Criando datasource...")
        ds_uid = manager.create_datasource("postgres-porto-uid")
        manager.debug_datasource(ds_uid)
        
        # 2. Verifica se foi criado
        try:
            response = api.get(f"/datasources/uid/{ds_uid}")
            ds_info = response.json()
            logger.info(f"✅ Datasource criado: {ds_info.get('name')}")
            
            # Verifica secureJsonFields
            secure_fields = ds_info.get("secureJsonFields", {})
            logger.info(f"🔐 Secure fields: {secure_fields}")
            
        except Exception as e:
            logger.error(f"❌ Datasource não encontrado: {e}")
            return 1
        
        # 3. Cria dashboard simples
        logger.info("2. Criando dashboard...")
        ...
        print(f"💡 Dica: Verifique se 'secureJsonFields' mostra 'password: true'")
        print(f"{'='*60}\n")
...
def execute():
    ...
    logger.info("🚀 INICIANDO CONFIGURAÇÃO DE EMERGÊNCIA")
    ...
    try:
        ...
        # Usa UID fixo - NÃO tenta criar datasource
        ds_uid = "postgres-porto-uid"
        logger.info(f"🔌 Usando datasource: {ds_uid}")
        
        # Cria dashboard diretamente
        ...
        print("📊 O dashboard foi criado com sucesso")
        print("🔌 AGORA CRIE O DATASOURCE MANUALMENTE:")
        print("1. Acesse: http://grafana_dashboard:3000")
        print("2. Vá em: Configuration → Data Sources")
        print("3. Clique em 'Add data source'")
        print("4. Selecione 'PostgreSQL'")
        print("5. Preencha:")
        print("   - Name: PostgreSQL-Porto-Operacional")
        print("   - Host: postgres_database:5432") 
        print("   - Database: grafana_database")
        print("   - User: grafana_admin")
        print("   - Password: secure_password_123")
        print("   - SSL Mode: disable")
        print("6. Clique em 'Save & Test'")
        print("7. Os painéis começarão a mostrar dados!")
        print(f"🔗 URL do Dashboard: {Config_grafana.URL}/d/Porto")
        print(f"{'='*70}\n")
